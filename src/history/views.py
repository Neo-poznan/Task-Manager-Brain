...
class HistoryView(TitleMixin, LoginRequiredMixinWithRedirectMessage, UserEntityMixin, View):
    use_case = HistoryUseCase(HistoryDatabaseRepository(Task, History, SharedHistory, connection))
...
class ShareHistoryView(UserEntityMixin, View):
    ...
    def post(self, request):
        try:
            ...
            return HttpResponseBadRequest(
                '''
                <h1>400</h1>
                <p>Для запроса истории в ссылке должны быть переданы query-параметры, которые должны включать временной интервал, по которому будет сохранена история!</p>
                '''
            )
        try:
            history_query_params_validator(from_date, to_date)
        ...
        shared_history_link = self.use_case.save_user_shared_history(self.get_user_entity(), from_date, to_date)   
    ...
    def get(self, request):
        try:
            ...
            return HttpResponseNotFound('<h1>404 Not Found</h1><p>Такой сохраненной истории не существует</p>')
    

class GetUserSharedHistories(LoginRequiredMixinWithRedirectMessage, UserEntityMixin, TitleMixin, ListView):
    title = 'Мои ссылки'
...
class SharedHistoryDeletionView(LoginRequiredMixinWithRedirectMessage, UserEntityMixin, DeleteView):
    use_case = HistoryUseCase(HistoryDatabaseRepository(Task, History, SharedHistory, connection))
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            return HttpResponseForbidden('<h1>403 Forbidden</h1><p>Вы пытаетесь удалить историю другого пользователя</p>')

    def delete(self, request, *args, **kwargs):
        self.use_case.delete_user_shared_history_by_key(self.kwargs.get('history_key'), self.get_user_entity())
        return JsonResponse({'redirect_url': reverse_lazy('history:user_shared_histories')}, status=203)


class HistoryDeletionView(LoginRequiredMixinWithRedirectMessage, UserEntityMixin, DeleteView):
    use_case = HistoryUseCase(HistoryDatabaseRepository(Task, History, SharedHistory, connection))
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            return HttpResponseNotFound('<h1>404 Not Found</h1><p>Такой записи в истории не существует</p>')
        except PermissionError:
            return HttpResponseForbidden('<h1>403 Forbidden</h1><p>Вы пытаетесь удалить запись в истории другого пользователя</p>')

    def delete(self, request, *args, **kwargs):
        self.use_case.delete_user_history_by_id(self.kwargs.get('history_id'), self.get_user_entity())
